...
public class MissionHistoryServiceImpl implements MissionHistoryService {
    ...
    public MissionSubmitResponse submitMission(MissionSubmitServiceRequest request) {
        ...
        if (isReadyConditionSatisfied(missionHistory, category)) {
    ...
    @Override
    public MissionSubmitResponse approveMission(Long missionHistoryId) {
    ...
    private boolean isReadyConditionSatisfied(MissionHistory missionHistory, MissionCategoryStatus category) {
        return switch (category) {
            case LOCATION -> missionHistory.getGpsVerifyCount() != null && missionHistory.getGpsVerifyCount() >= 3;
            case REPLACE_ACTION -> missionHistory.getPhotoUrl1() != null
                    && missionHistory.getPhotoUrl2() != null
                    && missionHistory.getPhotoUrl3() != null;
            case HOLD -> missionHistory.getAbstinenceTime() != null && missionHistory.getAbstinenceTime() > 0;
        };
    }
}
